...
def main():
    ...
    # 获取未背单词表中单词数量
    word_count = unmemorized.get_unmemorized_num(book_id)
    print(word_count)

    # 从未背词库中随机抽取并返回单词
    random_word = unmemorized.random_get_word_unmemorized(book_id)
    if random_word:
    ...
    # 删除未背词库中的指定单词
    unmemorized.delete_unmemorized_word(random_word, book_id)

    # 增加单词到已背单词表中
    memorized.add_memorized_word(random_word, book_id, date.today())
