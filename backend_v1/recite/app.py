...
recite_blueprint = Blueprint('recite', __name__)
CORS(recite_blueprint)  # 允许所有域名的跨域请求

@recite_blueprint.route('/api/hello', methods=['GET'])
def hello():
    return jsonify({'message': 'Hello from Flask!'})

@recite_blueprint.route('/api/get-unmemorized-num', methods=['GET'])
def get_unmemorized_num_endpoint():
    number = get_unmemorized_num()
    return jsonify({'number': number})

@recite_blueprint.route('/api/get-memorized-num', methods=['GET'])
def get_memorized_num_endpoint():
    number = get_memorized_num()
    return jsonify({'number': number})

@recite_blueprint.route('/api/random-get-word-unmemorized', methods=['GET'])
def random_get_word_unmemorized_endpoint():
    word = random_get_word_unmemorized()
    return jsonify({'word': word})

@recite_blueprint.route('/api/get-word-example', methods=['GET'])
def get_word_example_endpoint():
    word = request.args.get('word')
    example = get_word_example(word)
    return jsonify({'example': example})

@recite_blueprint.route('/api/get-word-meaning', methods=['GET'])
def get_word_meaning_endpoint():
    word = request.args.get('word')
    meaning = get_word_meaning(word)
    return jsonify({'meaning': meaning})

@recite_blueprint.route('/api/get-word-phonetic', methods=['GET'])
def get_word_phonetic_endpoint():
    word = request.args.get('word')
    usphone, ukphone = get_word_phonetic(word)
    return jsonify({'usphone': usphone, 'ukphone': ukphone})

@recite_blueprint.route('/api/add-memorized-word', methods=['POST'])
def add_memorized_word_endpoint():
    word = request.json.get('word')
    add_memorized_word(word, date.today())
    return '', 204

@recite_blueprint.route('/api/delete-unmemorized-word', methods=['POST'])
def delete_unmemorized_word_endpoint():
    word = request.json.get('word')
    delete_unmemorized_word(word)
    return '', 204

@recite_blueprint.route('/api/transfer-to-unmemorized', methods=['POST'])
def transfer_to_unmemorized_endpoint():
    word = request.json.get('word')
    transfer_to_unmemorized(word)
    return '', 204

@recite_blueprint.route('/api/clear-unmemorized-vocabulary', methods=['POST'])
def clear_unmemorized_vocabulary_endpoint():
    clear_unmemorized_vocabulary()
    return '', 204

@recite_blueprint.route('/api/get_word_count', methods=['GET'])
