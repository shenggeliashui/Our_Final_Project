...
import calendar
from db_connection import get_connection  # 使用绝对路径导入
...
def get_word_count_and_levels(year, month):
    ...
    try:
        ...
        if connection:
            ...
            query = """
            SELECT DAY(memorized_date) as day, COUNT(*) as count
            FROM memorized_words
            WHERE YEAR(memorized_date) = %s AND MONTH(memorized_date) = %s
            GROUP BY DAY(memorized_date)
            """
