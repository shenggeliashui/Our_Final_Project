...
def change_book(new_book_id):
    ...
    memorized.clear_memorized_vocabulary()
    unmemorized.clear_unmemorized_vocabulary()

    #重新导入未背词库表
    unmemorized.transfer_to_unmemorized(book_id)
...
# 根据单词从完整词库中获取并返回字典
def get_word_meaning(word,use_book_id):
    # 连接到数据库
    db = pymysql.connect(host="localhost", user="root", password="123456", database="memorized", charset="utf8")
    cursor = db.cursor()  # 创建一个游标对象
...
def get_word_example(word,use_book_id):
    # 连接到数据库
    db = pymysql.connect(host="localhost", user="root", password="123456", database="memorized", charset="utf8")
    cursor = db.cursor()  # 创建一个游标对象
