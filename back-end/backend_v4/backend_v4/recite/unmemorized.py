...
# 从未背词库中随机抽取并返回单词
def random_get_word_unmemorized(book_id):
    # 连接到数据库
    db = pymysql.connect(host="localhost", user="root", password="123456", database="memorized", charset="utf8")
    cursor = db.cursor()  # 创建一个游标对象
...
# 删除未背词库中的指定单词
def delete_unmemorized_word(word, book_id):
    # 连接到数据库
    db = pymysql.connect(host="localhost", user="root", password="123456", database="memorized", charset="utf8")
    cursor = db.cursor()  # 创建一个游标对象

    # 删除指定单词
    delete_word = "DELETE FROM unmemorized_words WHERE word = %s AND book_id = %s"
    cursor.execute(delete_word, (word, book_id))
...
def transfer_to_unmemorized(book_id):
    ...
    query = "INSERT IGNORE INTO unmemorized_words (word, book_id) SELECT word, bookId FROM words WHERE bookId = %s"
    cursor.execute(query, (book_id,))
