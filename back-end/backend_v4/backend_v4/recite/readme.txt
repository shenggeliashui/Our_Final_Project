...
6/22/22:46
v5 edited by 付芷怡
1.main.py中修改initialize_tables()函数，在已背词表中增加属性book_id用于记录单词从哪本书来的，并以book_id和word为混合主键
2.memorized.py中修改函数add_memorized_word，增加了属性book_id作为输入参数之一
3.test.py中对函数add_memorized_word的参数输入进行了修改，加上了book_id

6/24/21:47
v6 edited by 付芷怡
1.main.py中修改initialize_tables()函数，在未背词表中增加属性book_id用于记录单词从哪本书来的，并以book_id和word为混合主键
2.unmemorized.py中修改函数get_unmemorized_num，random_get_word_unmemorize，delete_unmemorized_word增加了属性book_id作为输入参数之一
3.test.py中对上述几个函数的参数输入进行了修改，加上了book_id
