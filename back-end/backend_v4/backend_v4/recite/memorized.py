...
# 根据书名获取已背单词表中单词数量
def get_memorized_num_with_id(book_id):
    # 连接到数据库
    db = pymysql.connect(host="localhost", user="root", password="123456", database="memorized", charset="utf8")
    cursor = db.cursor()  # 创建一个游标对象

    query = "SELECT COUNT(*) FROM memorized_words WHERE book_id = %s"
...
# 增加单词到已背单词表中
def add_memorized_word(word, book_id, date):
    # 连接到数据库
    db = pymysql.connect(host="localhost", user="root", password="123456", database="memorized", charset="utf8")
    cursor = db.cursor()

    # 插入新单词和日期
    add_word = "INSERT IGNORE INTO memorized_words (word, book_id, memorized_date) VALUES (%s, %s, %s)"
    data_word = (word, book_id, date)
